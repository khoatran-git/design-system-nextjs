 ...
 * Used for: styles list, navigation
 */
export const stylesQuery = `
  *[_type == "style" && status == "published"] 
  | order(order asc, title asc) {
    _id,
    title,
    slug,
...
/**
 * Fetch specific style by slug with full details
 * Used for: style detail page
 * Parameters: { slug: "colors" }
 */
export const styleBySlugQuery = `
  *[_type == "style" && slug.current == $slug && status == "published"][0] {
    _id,
    title,
    slug,
    description,
 ...
 * Used for: category-specific style pages
 * Parameters: { styleCategory: "Colors" }
 */
export const stylesByCategoryQuery = `
  *[_type == "style" && styleCategory == $styleCategory && status == "published"] 
  | order(order asc, title asc) {
    _id,
    title,
    slug,
